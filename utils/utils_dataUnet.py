# -*- coding: utf-8 -*-
"""
Created on Thu May 16 01:25:11 2024
...
class DatasetCSV(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        target = self.loader(path_target.replace('\\', os.sep))
   
        if self.transform is not None:
            sample, target = self.transform(sample, target)

        pos_ct1 = self.get_samples_ctpos(index)[0]
        pos_ct2 = self.get_samples_ctpos(index)[1]
        pos_x = self.get_samples_ctcapos(index)[2]
    
        if self.return_paths:
            return sample, target, path
        else:
            return sample, target, pos_ct1, pos_ct2, pos_x
    ...
    def make_dataset(self, rootdirlist, reset_samples=False):
        ...
        for rootdir in rootdirlist:
            ...
            gt_dlist = readCsvDataLists(rootdir, targetstr=['Data', 'Target', 'CT_pos', 'CTCA_pos', 'Fold'])
            gt_imglist = gt_dlist[0]
            gt_tgtlist = gt_dlist[1]
            gt_posctlist = gt_dlist[2]
            gt_posctcalist = gt_dlist[3]
            gt_fldlist = gt_dlist[4]
