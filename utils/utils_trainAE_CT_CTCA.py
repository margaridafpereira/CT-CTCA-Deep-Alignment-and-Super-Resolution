...
class DataloaderIterator(object):
    ...
    def __call__(self, model, optimizer, epoch):
        ...
        for ii, (ct, ctca) in enumerate(self.dataloader):
            ...
            masked_output, masked_ctca = mask(output, ctca)  
            lossCTCA = self.calcLoss(masked_output, masked_ctca)
            
            loss= 0.5 *lossCT + 0.5 * lossCTCA
            ...
            if self.mode == 'val':
                self.predictions.append(ct, output, target=ctca)
...
def mask(output, ctca):
    mask = (output < 0) & (ctca < 0)
    masked_output = output * mask.float()
    masked_ctca = ctca * mask.float()
    return masked_output, masked_ctca
