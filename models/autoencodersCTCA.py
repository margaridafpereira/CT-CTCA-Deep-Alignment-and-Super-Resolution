# -*- coding: utf-8 -*-
"""
Created on Thu Feb 29 09:38:00 2024
...
class _ResNetAE(nn.Module):
    def __init__(self, downblock, upblock, num_layers, n_classes):
        ...
        self.conv1 = nn.Conv3d(2, 64, kernel_size=(7,7,1), stride=2, padding=3, bias=False)
        self.bn1 = nn.BatchNorm3d(64)
        self.relu = nn.ReLU()
        self.maxpool = nn.MaxPool3d(kernel_size=(3,3,1), stride=2, padding=1)
        ...
        upsample = nn.Sequential(
            nn.ConvTranspose3d(self.in_channels, 64, kernel_size=(1,1,3), stride=2, bias=False, output_padding=1),
            nn.BatchNorm3d(64),
        )
        self.uplayer_top = DeconvBottleneck(self.in_channels, 64, 1, 2, upsample)

        self.conv1_1 = nn.ConvTranspose3d(64, n_classes, kernel_size=(1,1,3), stride=1, bias=False)
    ...
    def _make_downlayer(self, block, init_channels, num_layer, stride=1):
        ...
        if stride != 1 or self.in_channels != init_channels * block.expansion:
            downsample = nn.Sequential(
                nn.Conv3d(self.in_channels, init_channels * block.expansion, kernel_size=(1,1,3), stride=stride, bias=False),
                nn.BatchNorm3d(init_channels * block.expansion),
            )
        layers = []
    ...
    def _make_up_block(self, block, init_channels, num_layer, stride=1):
        ...
        if stride != 1 or self.in_channels != init_channels * 2:
            upsample = nn.Sequential(
                nn.ConvTranspose3d(self.in_channels, init_channels * 2, kernel_size=(1,1,3), stride=stride, bias=False, output_padding=1),
    ...
    def decode(self, x, image_size=[10, 2, 512, 512]):
        x = self.uplayer1(x)
        x = self.uplayer2(x)
        x = self.uplayer3(x)
        x = self.uplayer4(x)
    ...
    def forward(self, x):
        #print(x.shape)
        z = self.encode(x)
        x = self.decode(z, x.size())
        return x



def ResNetAE50(**kwargs):
    return _ResNetAE(Bottleneck, DeconvBottleneck, [3, 4, 6, 3], 2, **kwargs)  


def ResNetAE101_CTCA(**kwargs):
    return _ResNetAE(Bottleneck, DeconvBottleneck, [3, 4, 23, 2], 2, **kwargs)  
...
#         # Carregar os pesos ajustados no modelo
