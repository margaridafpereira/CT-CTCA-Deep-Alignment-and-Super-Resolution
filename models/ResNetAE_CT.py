...
class _ResNetAE(nn.Module):
    def __init__(self, in_channels, downblock, upblock, num_layers, n_classes):
        ...
        self.conv1_1 = nn.ConvTranspose2d(64, 2, kernel_size=1, stride=1, bias=False)
    ...
    def decode(self, x, image_size=[1, 1, 512, 512]):
        x = self.uplayer1(x)
        x = self.uplayer2(x)
        x = self.uplayer3(x)
        x = self.uplayer4(x)
...
def ResNetAE101(**kwargs):
    return _ResNetAE(Bottleneck, DeconvBottleneck, [3, 4, 23, 2], n_classes=2, **kwargs)
...
def download(url, filename):
    with open(filename, 'wb') as f:
        ...
        if total is None:
            ...
            downloaded = 0
            total = int(total)
            for data in response.iter_content(chunk_size=max(int(total / 1000), 512 * 512)):
                downloaded += len(data)
                f.write(data)
