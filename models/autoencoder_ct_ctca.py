...
class Autoencoder_CT_CTCA(nn.Module):
    def __init__(self,
                 in_channels: int = 1,
                 out_channels: int = 1,
                 n_blocks: int = 3,
        ...
        for i in range(self.n_blocks):
            ...
            down_blockCT = DownBlock(in_channels=num_filters_in,
        ...
        for i in range(self.n_blocks):
            ...
            down_blockCTCA = DownBlock(in_channels=num_filters_in,
                                   out_channels=num_filters_out,
                                   pooling=pooling,
                                   activation=self.activation,
                                   normalization=self.normalization,
        ...
        for i in range(n_blocks - 1):
            num_filters_in = (2*num_filters_out) if i == 0 else num_filters_out
    ...
    def forward(self, ct: torch.tensor, ctca:torch.tensor):
        ...
        combined = self.conv_final(combined)
        tanh_activation = torch.nn.Tanh()
        ct_output = tanh_activation(combined) * 1000.0 
