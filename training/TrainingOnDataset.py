...
def main(model_params, path_results):
    ...
    data = utils.utils_dataUnet.DatasetCSV(root=traincsvfpaths, transform=image_transform)
    ...
    #for k_ind in [2]: # Train single fold
        ...
        lossfunc = getattr(utils.utils_trainUnet, model_params['loss'])()
        accfunc = utils.utils_trainUnet.ClassAcc()

        valpred_fpath = os.path.join(path_results, 'val_examples')
        if not os.path.isdir(valpred_fpath):
            os.mkdir(valpred_fpath)
        valPredictions = utils.utils_trainUnet.dataPredictions(fpath=valpred_fpath)
        ...
        model, history = utils.utils_trainUnet.train(model, dataloaders['train'], dataloaders['val'],
...
if __name__ == '__main__':
    augm_params = {'img_range': [-1000, 1000],
                   ...
                   'rotate': 10,  
                   'flip': 0}
    model_params = {'model_folder': 'UNetSR_MSELoss_lr10-4',
                    'model_name': 'unet_weighted_average',
                    'batch_size': 4,
                    'size_input': 512,
                    'in_channels': 1,
                    'loss': 'MSELoss',
                    ...
                    'train_paths': [os.path.join('F:\MargaridaP', 'cardiac_ct_unet1.csv')],
                    'nfolds': 5,
