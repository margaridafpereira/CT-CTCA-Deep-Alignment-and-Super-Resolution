...
def main(model_params, path_results):
    ...
    data = utils.utils_dataCT.DatasetCSV(root=traincsvfpaths, transform=image_transform)
    print('Found {} samples for training.'.format(len(data.samples)))

    nfolds = model_params['nfolds']
    for k_ind in range(nfolds):
    #for k_ind in [0]: # Train single fold
        ...
        lossfunc = getattr(utils.utils_trainCT, model_params['loss'])()
        accfunc = utils.utils_trainCT.ClassAcc()

        valpred_fpath = os.path.join(path_results, 'val_examples')
        if not os.path.isdir(valpred_fpath):
            os.mkdir(valpred_fpath)
        valPredictions = utils.utils_trainCT.dataPredictions(fpath=valpred_fpath)
        ...
        model, history = utils.utils_trainCT.train(model, dataloaders['train'], dataloaders['val'],
...
if __name__ == '__main__':
    augm_params = {'img_range': [-1000, 1000],
                   ...
                   'rotate': 10,  
                   'flip': 0}
    model_params = {'model_folder': 'Final_AutoencoderCT_LR1e-4',
                    'model_name': 'autoencoder_ct',
                    'batch_size': 6,
                    'size_input': 512,
                    'in_channels': 1,
                    'loss': 'MSELoss',
