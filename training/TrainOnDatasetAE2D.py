# -*- coding: utf-8 -*-
"""
Created on Fri Jun  7 21:54:45 2024
...
def main(model_params, path_results):
    ...
    for k_ind in [4]: # Train single fold
        ...
        lossfunc = getattr(utils.utils_trainAE2D, model_params['loss'])()
        #accfunc = utils.utils_trainAE2D.ClassAcc()

        valpred_fpath = os.path.join(path_results, 'val_examples')
        if not os.path.isdir(valpred_fpath):
            os.mkdir(valpred_fpath)
        valPredictions = utils.utils_trainAE2D.dataPredictions(fpath=valpred_fpath)
        ...
        model, history = utils.utils_trainAE2D.train(model, dataloaders['train'], dataloaders['val'],
        ...
        with open(os.path.join(path_results, 'fold{}history.csv'.format(k_ind)), 'w',
            ...
            filewriter.writerow(['train_loss', 'valid_loss'])
...
if __name__ == '__main__':
    augm_params = {'img_range': [-1000, 1000],
                   ...
                   'rotate': 10,  
                   'flip': 0}
    model_params = {'model_folder': 'AutoencoderAE2D_lr10-4',
                    'model_name': 'autoencoder2D',
                    'batch_size': 2,
                    'size_input': 512,
                    'in_channels': 1,
                    'loss': 'MSELoss',
                    ...
                    'n_epochs': 100,
                    'learning_rate': 1e-4,
                    'train_paths': [os.path.join('D:\MargaridaP\pericardial_segmentation\data', 'cardiac_ct_ctcaAE1.csv')],
                    #'train_paths': [os.path.join('F:\MargaridaP', 'cardiac_ct_ctcaAE.csv')],
                    'nfolds': 5,
