...
def main(model_params, path_results):
    ...
    data = utils.utils_dataCTCA.DatasetCSV(root=traincsvfpaths, transform=image_transform)
    ...
    #for k_ind in [2]: # Train single fold
        ...
        dataloaders = {
            'train': DataLoader(data, batch_size=model_params['batch_size'], in_channels=model_params['in_channels'], sampler=train_sampler, shuffle=False),
            'val': DataLoader(data, batch_size=model_params['batch_size'], in_channels=model_params['in_channels'], sampler=val_sampler, shuffle=False)}
        ...
        lossfunc = getattr(utils.utils_trainCTCA, model_params['loss'])()
        accfunc = utils.utils_trainCTCA.ClassAcc()
    
        valpred_fpath = os.path.join(path_results, 'val_examples')
        if not os.path.isdir(valpred_fpath):
                os.mkdir(valpred_fpath)
        valPredictions = utils.utils_trainCTCA.dataPredictions(fpath=valpred_fpath)
        ...
        model, history = utils.utils_trainCTCA.train(model, dataloaders['train'], dataloaders['val'],
...
if __name__ == '__main__':
    augm_params = {'img_range': [-1000, 1000],
                   ...
                   'rotate': 10,  
                   'flip': 0}
    model_params = {'model_folder': 'AutoencoderCTCA_LR1e-4_3blocks',
                    'model_name': 'autoencoder_ctca',
                    'batch_size': 1,
                    'size_input': 512,
                    'in_channels': 1,
                    'loss': 'MSELoss',
                    ...
                    'n_epochs': 300,
                    'learning_rate': 1e-4,
                    'train_paths': [os.path.join(utils.utils_paths.datapaths['cardiac_ct'], 'cardiac_ct_ctca2.csv')],
                    'nfolds': 5,
